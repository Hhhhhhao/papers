...
class GAN:
    ...
    def build_discriminator(self):
        ...
        x = ConvSN2D(ch, kernel_size=5, strides=1, padding='same', spectral_norm=True)(x_input)
        # x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.2)(x)

        for i in range(self.layer_num // 2):
            x = ConvSN2D(ch * 2, kernel_size=5, strides=2, padding='same', spectral_norm=True)(x)
            # x = BatchNormalization()(x)
            x = LeakyReLU(alpha=0.2)(x)
            ch = ch * 2
        ...
        x = Flatten()(x)
        x = DenseSN(1024, spectral_norm=True)(x)
        # x = BatchNormalization()(x)
        x = LeakyReLU(alpha=0.2)(x)
        x = Dropout(0.25)(x)
