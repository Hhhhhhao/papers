...
class GAN:
    def __init__(self, dataset='MNIST', upsampling=False):
        ...
        if upsampling:
            self.sample_dir = check_folder(dataset + '_upsampling_samples/')
            self.weights_dir = check_folder(dataset + '_upsampling_weights/')
        elif not upsampling:
            self.sample_dir = check_folder(dataset + '_samples/')
            self.weights_dir = check_folder(dataset + '_weights/')
        else:
            raise ValueError("upsampling:{} is not valid".format(upsampling))
    ...
    def build_generator(self):
        ...
        if self.upsampling:
            ...
            x = Conv2DTranspose(128, kernel_size=5, strides=2, padding='same', name='g_deconv2')(x)
            x = BatchNormalization(name='g_bn2')(x)
            x = Activation('relu', name='g_ac2')(x)
            x = Conv2D(64, kernel_size=5, strides=1, padding='same', name='g_conv3')(x)
            x = BatchNormalization(name='g_bn3')(x)
            ...
            x = Conv2DTranspose(32, kernel_size=5, strides=2, padding='same', name='g_deconv4')(x)
            x = BatchNormalization(name='g_bn4')(x)
            x = Activation('relu', name='g_ac4')(x)
            x = Conv2D(3, kernel_size=5, strides=1, padding='same', name='g_conv5')(x)
            x = BatchNormalization(name='g_bn5')(x)
            ...
            x = Conv2D(self.channels, kernel_size=3, strides=1, padding='same', name='g_deconv6')(x)
            x_output = Activation('sigmoid', name='g_ac6')(x)
    ...
    def build_discriminator(self):
        ...
        x = Conv2D(16, kernel_size=5, strides=2, padding='same', name='d_conv1')(x_input)
        x = BatchNormalization(name='d_bn1')(x)
        x = LeakyReLU(alpha=0.2, name='d_ac1')(x)
        x = Conv2D(32, kernel_size=5, strides=1, padding='same', name='d_conv2')(x)
        x = BatchNormalization(name='d_bn2')(x)
        x = LeakyReLU(alpha=0.2, name='d_ac2')(x)
        x = Conv2D(64, kernel_size=5, strides=2, padding='same', name='d_conv3')(x)
        ...
        for epoch in range(epochs):
            ...
            for t in range(len(train_generator)):
                ...
                if t%sample_intervals ==0 or t == len(train_generator)-1:
                    print("iteration:%d [D loss: %a, acc.: %.2f%%] [G loss: %g] "
            ...
            # Plot the progress
            print("validation [D loss: %a, acc.: %.2f%%] [G loss: %g] "
...
if __name__ == '__main__':
    dataset = 'MNIST'
    upsampling = True
    gan = GAN(dataset, upsampling)
    loss_dir = check_folder(dataset + '_losses/')
    # set epochs to 30 get good results
    losses = gan.train(epochs=1, batch_size=64, label_smooth=True, sample_intervals=50)
